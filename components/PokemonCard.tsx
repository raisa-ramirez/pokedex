...
import { Card, CardActions, CardContent, CardMedia, LinearProgress, List, ListItem, ListItemIcon, ListItemText, Typography } from "@mui/material";
    ...
    useEffect(() => {
        if(data!==''){
            let info = {
                ...
                stats: transformArray(data.stats.map((item:any) => ({name: item.stat.name, value: item.base_stat})))
            }
            setPokemon(info)
        }         
    }, [data])     
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <Card sx={{ maxWidth: 400, padding: '1rem', height: 440}}>
                        <CardMedia
                        component="img"
                            sx={{ width: '50%', margin: 'auto', marginY: '1.2rem', maxHeight: '45%', objectFit:'contain'}}
                            title="pokemon image"
                            image={pokemon.image}
                        />
                        <CardContent>
                ...
                <div>
                    <Card sx={{ maxWidth: 400, height: 440}}>
                        ...
                        <List>
                            {
                                pokemon.stats.map((stat:any) => <ListItem key={stat.name}>
                                    <ListItemIcon>
                                        <Image src={`/${stat.image}`} width={20} height={20} alt="icon"/>
                                    </ListItemIcon>
                                    <ListItemText 
                                    className="text-secondary"
                                    primary={`${stat.name}: ${stat.value}`}
                                    secondary={
                                        <LinearProgress variant="determinate" value={stat.value} sx={{height: '10px', borderRadius: '20px'}} color="info"/>
                                    }>
                                    </ListItemText>
                                </ListItem>)
                            }                            
